...
def process_data(raw_data):
    ...
    # preprocess data
    data['pop_up_col'] = 'name: ' + data.location_name + '；' + 'type:' + data.sg_c__sub_category + '；' + 'stay time: ' + \
    ...
    data["median_dwell_hour"] = data["sg_wp__median_dwell"] / 60

    # aggravate the business type of data by naics code
    data['sg_c__naics_code'] = data.sg_c__naics_code.astype(int)
    ...
    data['poi_cbg']=data['poi_cbg'].astype('int64')
    data['tract_id']=data['poi_cbg'].astype(str).str[:11].astype('int64')
